...
Single profileは、案件の開始・統制・日常更新を最小限の8シートで扱います。「プロジェクト憲章」は目的、価値、スコープ、受入基準、スポンサー、責任者、予算基準線／予測／実績、レビュー日、テーラリング方針を保持します。「作業項目」の種別「マイルストーン」を「ダッシュボード」が期限順に集計し、「RAIDログ」はリスク・前提・課題・依存関係を、「ステークホルダー」は関係者のコミュニケーションを管理します。

「意思決定・変更ログ」は記録種別（変更要求／意思決定）と変更管理状態（起票、影響分析中、承認、却下、実施中、完了）を持ちます。分離された予算・資源台帳、変更台帳、定型ステータスレポートは意図的に後続拡張とし、MVPでは「プロジェクト憲章」「意思決定・変更ログ」「作業項目」に集約します。
