...
Single profileは、案件の開始・統制・日常更新を9シートで扱います。「プロジェクト憲章」は目的、価値、スコープ、受入基準、スポンサー、責任者、予算基準線／予測／実績、レビュー日、テーラリング方針を保持します。「WBS」の種別「マイルストーン」を「ダッシュボード」が期限順に集計し、「RAIDログ」はリスク・前提・課題・依存関係を、「ステークホルダー」は関係者のコミュニケーションを管理します。

「WBS」は成果物／作業パッケージを階層化し、基準日程、実績日程、先行タスク、進捗率、工数、遅延理由を一元管理します。「ガントチャート」はWBSを参照する表示専用ビューで、日次／週次と4〜52期間を切り替え、計画・実績・遅延・ブロック・マイルストーンを条件付き書式で可視化します。
