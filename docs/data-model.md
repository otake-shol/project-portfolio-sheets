...
Single profileは、案件の開始・統制・日常更新を10シートで扱います。「プロジェクト憲章」は目的、価値、スコープ、受入基準、スポンサー、責任者、予算基準線／予測／実績、レビュー日、テーラリング方針を保持します。「WBS」の種別「マイルストーン」を「ダッシュボード」が期限順に集計し、「RAIDログ」はリスク・前提・課題・依存関係を、「ステークホルダー」は関係者のコミュニケーションを管理します。

「WBS」は成果物／作業パッケージを階層化し、基準日程、実績日程、先行タスク、進捗率、工数、遅延理由を一元管理します。「概要スケジュール」は階層1とマイルストーンを最大18件まで抽出する関係者向けビューで、週次／月次と4〜18期間を切り替え、主要日程・完了・遅延／ブロック・マイルストーンを一枚で可視化します。「ガントチャート」はWBSを参照する内部管理用ビューで、日次／週次と4〜52期間を切り替え、計画・実績・遅延・ブロック・マイルストーンを条件付き書式で可視化します。

「意思決定・変更ログ」は記録種別（変更要求／意思決定）と変更管理状態（起票、影響分析中、承認、却下、実施中、完了）を持ちます。分離された予算・資源台帳、変更台帳、定型ステータスレポートは意図的に後続拡張とし、MVPでは「プロジェクト憲章」「意思決定・変更ログ」「WBS」に集約します。
