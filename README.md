...
Apps Scriptでは`createSingleProjectWorkbook`を実行します。生成するシートは「ダッシュボード」「ガイド」「プロジェクト憲章」「WBS」「ガントチャート」「RAIDログ」「ステークホルダー」「意思決定・変更ログ」「マスター」です。マイルストーンは「WBS」の種別で管理し、予算基準線・予測・実績、全体状態、最終レビューは「プロジェクト憲章」で管理します。変更要求と意思決定は「意思決定・変更ログ」に記録します。
...
「WBS」は階層、基準開始／終了日、実績開始／終了日、先行タスク、進捗率、遅延理由を保持する唯一の入力元です。「ガントチャート」はWBSを数式参照し、日次／週次、4〜52期間を切り替えて、計画・実績・遅延・ブロック・マイルストーンを表示します。
