function verifySingleProjectWorkbook_(ss) {
  ...
  const dashboard = ss.getSheetByName(SINGLE_SHEET_NAMES.dashboard); const work = ss.getSheetByName(SINGLE_SHEET_NAMES.workItems); const raid = ss.getSheetByName(SINGLE_SHEET_NAMES.raid); const stakeholders = ss.getSheetByName(SINGLE_SHEET_NAMES.stakeholders); const decision = ss.getSheetByName(SINGLE_SHEET_NAMES.decisions);
  ...
  [SINGLE_SHEET_NAMES.dashboard, SINGLE_SHEET_NAMES.workItems, SINGLE_SHEET_NAMES.raid, SINGLE_SHEET_NAMES.decisions].forEach(function(name) { report.conditionalFormats[name] = ss.getSheetByName(name).getConditionalFormatRules().length > 0; });
